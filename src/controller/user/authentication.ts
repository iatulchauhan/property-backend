...
export const signUp = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // let data = await sendSMSHelper(body.phoneNumber);
    // if (data?.data?.type == "success") {
    //   body.otp = JSON.parse(data?.config?.data)?.OTP;
    // }
...
export const login = async (req: Request, res: Response) => {
  ...
  reqInfo(req);
  try {
    let response: any = (await userModel.aggregate([{ $match: { phoneNumber: body.phoneNumber, isActive: true } },]))[0];

    if (!response)
      return res.status(400).json(await apiResponse(400, responseMessage?.invalidMobile, {}, {}));
    ...
    let otpConfig = await sendSMSHelper(body.phoneNumber);

    if (otpConfig?.data?.type == "success") {
      response = await userModel.findByIdAndUpdate({ _id: new ObjectId(response?._id), isActive: true }, { otp: JSON.parse(otpConfig?.config?.data)?.OTP }, { new: true });
    }
